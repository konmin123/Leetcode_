...
with arr[1], the larger integer wins and remains at position 0, and the smaller
...
class Solution:
    @staticmethod
    def get_winner_brutal_force(arr: List[int], k: int) -> int:
        win_strike = 0
        while win_strike < k:
            start = 1
            if arr[0] > arr[1]:
...
assert Solution.get_winner_brutal_force([1, 2, 3], 10) == 3
assert Solution.get_winner_brutal_force([2, 1, 3, 5, 4, 6, 7], 2) == 5
