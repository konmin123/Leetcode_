...
class Solution:
    ...
    def find_restaurant(list1: List[str], list2: List[str]) -> List[str]:
        ...
        for index_list_1, element in enumerate(list1):
            if element in list2:
                index_list_2 = list2.index(element)
                dict_values[element] = index_list_1 + index_list_2
        min_sum_index = min(dict_values.values())
        return [x[0] for x in dict_values.items() if x[1] == min_sum_index]


assert Solution.find_restaurant(
