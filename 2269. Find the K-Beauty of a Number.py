...
class Solution:
    ...
    def divisor_substrings(num: int, k: int) -> int:
        ...
        cur_l: int = 0
        cur_r = k
        while cur_r <= len(str_num):
            number = int(str_num[cur_l: cur_r])
...
assert Solution.divisor_substrings(430043, 2) == 2
assert Solution.divisor_substrings(240, 2) == 2
assert Solution.divisor_substrings(2, 1) == 1
