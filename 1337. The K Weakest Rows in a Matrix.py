...
row. A row i is weaker than a row j if one of the following is true:
...
class Solution:
    ...
    def k_weakest_rows(mat: List[List[int]], k: int) -> List[int]:
        power_rows = [(x, sum(y)) for x, y in enumerate(mat)]
        power_rows.sort(key=lambda x: x[1])
        return [x[0] for x in power_rows[:k]]
