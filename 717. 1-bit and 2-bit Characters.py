...
class Solution:
    ...
    def is_one_bit_character(bits: List[int]) -> bool:
        if bits[-1]:
            return False
        cur: int = 0
        max_index: int = len(bits) - 1
        while cur < max_index:
            if bits[cur] == 1:
                cur += 2
            else:
                cur += 1
        return True if cur <= max_index else False


assert Solution.is_one_bit_character([1, 0, 0])
assert not Solution.is_one_bit_character([1, 1, 1, 0])
assert not Solution.is_one_bit_character([1, 0, 0, 1])
