...
class Solution:
    ...
    def find_content_children(g: List[int], s: List[int]) -> int:
        output: int = 0
        for children_greed_factor in g:
            if children_greed_factor in s:
                index_cookie: int = s.index(children_greed_factor)
                output += 1
                s[index_cookie] = 0
            elif sum(s) > children_greed_factor:
                sum_cookies = 0
                for index, cookie in enumerate(s):
                    sum_cookies += cookie
                    s[index] = 0
                    if sum_cookies >= children_greed_factor:
                        output += 1
                        break
            elif sum(s) == children_greed_factor:
                output += 1
...
print(Solution.find_content_children([1, 2, 3], [1, 1]))
