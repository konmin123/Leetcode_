...
class Solution:
    ...
    def is_alien_sorted(words: List[str], order: str) -> bool:
        list_index: List[str] = ['' for _ in words]
        count_iterations = len(max(words, key=lambda x: len(x)))
        for iteration in range(count_iterations):
            for index_word, word in enumerate(words):
                try:
                    cur_letter: str = word[iteration]
                    letter_index: str = str(order.index(cur_letter) + 1)
                except IndexError:
                    letter_index: str = '0'
                list_index[index_word] = list_index[index_word] + letter_index
        return list_index == sorted(list_index)


assert Solution.is_alien_sorted(["hello", "leetcode"],
                                "hlabcdefgijkmnopqrstuvwxyz")
assert not Solution.is_alien_sorted(["word", "world", "row"],
                                    "worldabcefghijkmnpqstuvxyz")
assert not Solution.is_alien_sorted(["apple", "app"],
