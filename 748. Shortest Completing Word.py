...
"caaacab", and "cbca". Return the shortest completing word in words.
It is guaranteed an answer exists. If there are multiple shortest completing
...
class Solution:
    ...
    def shortest_completing_word(license_plate: str, words: List[str]) -> str:
        ...
        for letter in license_plate:
            if letter.isalpha():
                template[letter.lower()] += 1
        for word in words:
            add = True
            for item in template.items():
                if item[0] not in word or word.count(item[0]) < item[1]:
                    add = False
                    break
            if add:
                output_list.append(word)
        return min(output_list, key=len)


assert Solution.shortest_completing_word(
