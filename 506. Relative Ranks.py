...
class Solution:
    ...
    def find_relative_ranks(score: List[int]) -> List[str]:
        output = score
        sorted_list_score = sorted(score)
        dict_str = {1: "Gold Medal", 2: "Silver Medal", 3: "Bronze Medal"}
        count = 1
        for result in sorted_list_score[::-1]:
            index = score.index(result)
            if count in dict_str:
                output[index] = dict_str[count]
            else:
...
assert Solution.find_relative_ranks([5, 4, 3, 2, 1]) == [
    "Gold Medal", "Silver Medal", "Bronze Medal", "4", "5"]

assert Solution.find_relative_ranks([10, 3, 8, 9, 4]) == [
