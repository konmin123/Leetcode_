...
k times. In one shift operation:
...
class Solution:
    ...
    def shift_grid(grid: List[List[int]], k: int) -> List[List[int]]:
        list_values = [value for list_in in grid for value in list_in]  # O(N)
        list_values = list_values[-k:] + list_values[:-k]  # O(1)
        m = len(grid)  # O(1)
        output = [list_values[i:i + m] for i in range(0, len(list_values), m)]
        return output


assert Solution.shift_grid(
