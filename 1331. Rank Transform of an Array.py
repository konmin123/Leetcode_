...
rank. If two elements are equal, their rank must be the same. Rank should be as
small as possible.
...
class Solution:
    ...
    def array_rank_transform(arr: List[int]) -> List[int]:
        rank_dict: dict = {}
        rank: int = 1
        for value in sorted(list(set(arr))):
            rank_dict[value] = rank
            rank += 1
        for index, value in enumerate(arr):
            arr[index] = rank_dict[value]
        return arr


assert Solution.array_rank_transform([40, 10, 20, 30]) == [4, 1, 2, 3]
assert Solution.array_rank_transform([100, 100, 100]) == [1, 1, 1]
assert Solution.array_rank_transform(
