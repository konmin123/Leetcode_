...
def remove_element(nums: array, val: int) -> int:
    index_for_replaced = -1
    current_index = 0
    while current_index - index_for_replaced <= len(nums):
        if nums[current_index] == val:
            nums[current_index], nums[index_for_replaced] = nums[index_for_replaced], nums[current_index]
            index_for_replaced -= 1
        else:
            current_index += 1
...
if __name__ == '__main__':
    print(remove_element(arr, val), arr)
